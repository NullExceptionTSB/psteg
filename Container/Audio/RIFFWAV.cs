...
namespace psteg.Container.Audio {
    ...
    public sealed class RIFFWAV : AudioContainer {
        ...
        protected override void Parse(byte[] rawdata) {
            ...
            Channels = format.nChannels == 1 ? Channels.Mono : Channels.Stereo;
            ...
            string datamark = new string(Encoding.ASCII.GetChars(rawdata, 12 + 4 + (int)format.dwFmtLen + 4, 4));
            int datalen = 0;
            if (datamark == "data")
                datalen = BitConverter.ToInt32(rawdata, 12 + 4 + (int)format.dwFmtLen + 4 + 4);
